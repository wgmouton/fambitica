...
export default {
  ...
  computed: {
    ...
    canCreateTasks () {
      ...
      const test = (this.group.leader && this.group.leader._id === this.user._id)
  ...
  mounted () {
    if (this.$route.path === '/party') this.$router.push('/party/tasks');
  ...
  methods: {
    async load () {
      ...
      //   if (this.group.type === 'party') this.$router.push('/party/tasks');
      //   return;
      // }
      const members = await this.$store.dispatch('members:getGroupMembers', { groupId: this.group._id });
      this.group.members = members;
      console.log(this);

      if (this.isParty && !this.user.preferences.tasks.mirrorGroupTasks[this.group._id]) {
        console.log(this.user.preferences);
        this.changeMirrorPreference(true);
      } else {
